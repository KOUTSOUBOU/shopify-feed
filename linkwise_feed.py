...
# Απλώς μετακίνησε γραμμές πάνω/κάτω ή σβήσε τες.
CATEGORY_PRIORITY = [
    # -- πιο ειδικές --
    "Στοματική Απόλαυση",
    "Ραβδιά Μασάζ",
    "Δονητές Δακτύλου",
    "Δονητές Διπλής Διέγερσης",
    "Δονητές Μικρού Μεγέθους",
    "G-spot",
    "Κλειτοριδικά",
    "Πρωκτικά",
    "Προϊόντα Αυνανισμού για Πέη",
    "Προφυλακτικά",
    "Λάδι Μασάζ",
    "Λιπαντικά",
    "Καθαρισμός",
    # -- πιο γενικές --
    "Δονητές",
    # -- τελευταία καταφυγή, πριν πέσουμε στο product_type --
    "Κόκκινο δωμάτιο",
    "Ευεξία",
    "Couples",
]
...
def get_collection_map():
    ...
    for c in collections:
        ...
        if title not in wanted:
            continue
        prods = _get_paginated(f"{base}/collections/{c['id']}/products.json?limit=250")
        for p in prods:
...
def pick_category(product, collection_map):
    ...
    for title in CATEGORY_PRIORITY:
        if title in cols:
            return title
